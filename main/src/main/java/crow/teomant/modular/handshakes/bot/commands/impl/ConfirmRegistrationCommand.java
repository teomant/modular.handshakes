...
public class ConfirmRegistrationCommand implements Command {
    ...
    public SendMessage processMessage(Message message) {
        ...
        Map<Integer, User> options = botState.getRegistrationOptions(message);
        ...
        if (StringUtils.isNumeric(message.getText()) && options.containsKey(Integer.parseInt(message.getText()))) {
            ...
            if (selected == 0) {
                ...
                user.setTelegramId(insertedUser.getTelegramId());

                userService.update(user);
            }

            botState.removeRegistrationOptions(message);
