...
public class ChooseToCommand implements Command {
    ...
    public SendMessage processMessage(Message message) {
        ...
        Map<Integer, User> options = relationCommandState.getToOptions(message);
        ...
        if (StringUtils.isNumeric(message.getText()) && options.containsKey(Integer.parseInt(message.getText()))) {
            ...
            if (relationCommandState.isChoosingFrom(message)) {

                relationCommandState.addTo(message, user);

                return new SendMessage(message.getChatId().toString(), CommandUtils.getOptionsMessage(
                    relationCommandState.getFromOptions(message)
                ));
            } else {
                userService.addRelation(
                    relationCommandState.getFrom(message).getId(),
                    user.getId(),
